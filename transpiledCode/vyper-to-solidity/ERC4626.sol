...
contract FromVyper {
    ...
    // ##### VYPER DECORATORS #####
    // @external
    constructor (ERC20 asset) external
    {
        asset = asset;
    }
    ...
    // @view
    // @external
    function name() external view returns (string[10])
    {
        return NAME;
    }
    ...
    // @view
    // @external
    function symbol() external view returns (string[5])
    {
        return SYMBOL;
    }
    ...
    // ##### VYPER DECORATORS #####
    // @external
    function deposit(uint256 assets, address receiver = msg.sender) external returns (uint256)
    {
    ...
    // ##### VYPER DECORATORS #####
    // @external
    function mint(uint256 shares, address receiver = msg.sender) external returns (uint256)
    {
        uint256 assets = _convertToAssets(shares);
        if (assets == 0 && asset.balanceOf(this) == 0) {
    ...
    // ##### VYPER DECORATORS #####
    // @external
    function withdraw(uint256 assets, address receiver = msg.sender, address owner = msg.sender) external returns (uint256)
    {
        uint256 shares = _convertToShares(assets);
        // NOTE: Vyper does lazy eval on `and`, so this avoids SLOADs most of the time
        if (shares == assets && totalSupply == 0) {
    ...
    // ##### VYPER DECORATORS #####
    // @external
    function redeem(uint256 shares, address receiver = msg.sender, address owner = msg.sender) external returns (uint256)
    {
        if (owner != msg.sender) {
            allowance[owner][msg.sender] -= shares;
        }
