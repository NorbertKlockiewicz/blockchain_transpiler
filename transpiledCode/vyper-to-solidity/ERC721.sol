interface ERC721Receiver {
    function onERC721Received(address _operator, address _from, uint256 _tokenId, bytes[1024] _data) external view returns (bytes4);
}
...
contract FromVyper {
    ...
    */
    function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes[1024] _data = bytes("")) external
    {
        _transferFrom(_from, _to, _tokenId, msg.sender);
        if (_to.is_contract) {
    ...
    */
    ...
    // @view
    // @external
    function tokenURI(uint256 tokenId) external view returns (string[132])
    {
