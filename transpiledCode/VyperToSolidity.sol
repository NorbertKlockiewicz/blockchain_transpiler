...
contract FromVyper {
    ...
    // ##### VYPER DECORATORS #####
    // @external
    constructor (address[5] _owners, int128 _threshold) private external
    {
    ...
    // ##### VYPER DECORATORS #####
    // @external
    function testEcrecover(bytes32 h, uint256 v, uint256 r, uint256 s) public external returns (address)
    {
    ...
    // @external
    // @payable
    function approve(int128 _seq, address to, uint256 _value, bytes[4096] data, uint256[3][5] sigdata) public external payable returns (bytes[4096])
    {
        ...
        bytes32 h2 = keccak256(concat(b"\x19Ethereum Signed Message:\n32", h));
    ...
    // @external
    // @payable
    fallback() private external payable
    {
    }
